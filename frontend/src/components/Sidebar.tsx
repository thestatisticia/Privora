...
function Icon({ d }: { d: string }) {
  return (
    <svg className="w-[18px] h-[18px] shrink-0 opacity-75" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function WalletBlock() {
  ...
  if (connected && address) {
    return (
      <div className="space-y-2">
        <p className="text-sm font-mono text-[var(--muted)] truncate px-1">{truncateAddress(address)}</p>
        <button type="button" onClick={disconnect} className="btn btn-secondary w-full text-sm py-2.5">
          Disconnect
        </button>
      </div>
    );
  ...
  return (
    <button type="button" onClick={connect} disabled={connecting} className="btn btn-primary w-full text-sm py-2.5">
    ...
    <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-7">
      {groups.map((group) => (
        <div key={group.heading}>
          <p className="px-2.5 mb-2 text-[11px] font-semibold uppercase tracking-widest text-[var(--muted)]">
            {group.heading}
          </p>
          <div className="space-y-0.5">
            {group.items.map((item) => {
              ...
              const cls = `flex items-center gap-3 px-2.5 py-2.5 rounded-md text-[15px] font-medium transition-colors ${
                active
...
export default function Sidebar() {
  ...
  return (
    <>
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-[16.5rem] flex-col shell-border-r bg-[var(--background)] z-40">
        <div className="px-4 h-[3.75rem] flex items-center shell-border-b">
          <Logo />
        </div>
        <NavList groups={groups} />
        <div className="p-3 shell-border-t space-y-2.5">
          <div className="flex items-center justify-between px-1">
            <span className="text-[11px] uppercase tracking-widest text-[var(--muted)] font-medium">Theme</span>
            <ThemeToggle />
          </div>
          <WalletBlock />
        </div>
